...
- **8 Rounds**: 8 unique grids with fixed true counts, each shown once
...
Each participant generates 8 rows of data with the following columns:
...
| `index_tour` | Integer | Round number (1-8) |
...
The game uses a fixed set of 8 true counts for consistency across participants:
`[37, 43, 48, 53, 58, 63, 68, 73]`
...
ROUNDS = 8               # Total rounds (one per matrix)
