...
- **10 Rounds**: 10 unique grids with fixed true counts, each shown once
...
Each participant generates 10 rows of data with the following columns:
...
| `index_tour` | Integer | Round number (1-10) |
| `id_verite` | Integer | Matrix ID (1-10) |
...
The game uses a fixed set of 10 true counts for consistency across participants:
`[32, 37, 43, 48, 53, 58, 63, 68, 73, 77]`
...
N_MATRICES = 10          # Number of unique grids
ROUNDS = 10              # Total rounds (one per matrix)
